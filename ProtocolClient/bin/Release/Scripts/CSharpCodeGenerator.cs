        ...
        public void Generate(ProjectInfo oProjectInfo, GeneratorSetting oGeneratorSetting, string sPath1, string sPath2)
        {
            ...
            foreach (var stitem in oProjectInfo.Structs)
            {
                ...
                foreach (var fieldItem in stitem.Fields)
                {
                    ...
                    {
                        TypeStr = CodeUtil.GetU3DTypeStr(fieldItem);
                        if (fieldItem.FieldLength == "*")
                        {
                            sbStructFile.Append(string.Format("        public List<{0}> {1} = new List<{0}>();\r\n", TypeStr, fieldItem.FieldName));
                        }
                        else
                        {
                            sbStructFile.Append(string.Format("        public {0}[] {1} = new {0}[{2}];\r\n", TypeStr, fieldItem.FieldName, fieldItem.FieldLength));
                        }
                    }
                    else
                    {
                ...
                foreach (var fieldItem in stitem.Fields)
                {
                    ...
                    else if (sTemp == "List")
                    {
                        ...
                        if (sTemp2 == "Pak")
                        {
                            ...
                            {
                                sbStructFile.Append(string.Format("            for (int i = 0; i < {0}; i++)\r\n", fieldItem.FieldLength));
                                sbStructFile.Append("            {\r\n");
                                sbStructFile.Append(string.Format("                st.{1}[i] = ({1}[i].Clone() as {0});\r\n", fieldItem.FieldType, fieldItem.FieldName));
                                sbStructFile.Append("            }\r\n");
                            }
                        }
                        else
                        {
                            ...
                            else
                            {
                                sbStructFile.Append(string.Format("            for (int i = 0; i < {0}; i++)\r\n", fieldItem.FieldLength));
                                sbStructFile.Append("            {\r\n");
                                sbStructFile.Append(string.Format("                st.{1}[i] = {1}[i];\r\n", fieldItem.FieldType, fieldItem.FieldName));
                                sbStructFile.Append("            }\r\n");
                            }
                        }
                    }
                ...
                foreach (var fieldItem in stitem.Fields)
                {
                    ...
                    else if (sTemp == "List")
                    {
                        if (fieldItem.FieldLength == "*")
                        {
                            sbStructFile.Append(string.Format("                oByteArray.WriteUShort((ushort){0}.Count);\r\n", fieldItem.FieldName));
                ...
                foreach (var fieldItem in stitem.Fields)
                {
                    ...
                    else if (sTemp == "List")
                    {
                        if (fieldItem.FieldLength == "*")
                        {
                            sbStructFile.Append(string.Format("                int {0}Count = (int)oByteArray.ReadUShort();\r\n", fieldItem.FieldName));
                            sbStructFile.Append(string.Format("                for (int i = 0; i < {0}Count; i++)\r\n", fieldItem.FieldName));
                            sbStructFile.Append("                {\r\n");
                            string sTemp2 = CodeUtil.GetU3DTypeStr(fieldItem, true);
                            if (sTemp2 == "Pak")
                            {
                                ...
                                sbStructFile.Append("                    obj.Serializtion(oByteArray, bSerialize);\r\n");
                        ...
                        else
                        {
                            ...
                            if (sTemp2 == "Pak")
                            {
                                sbStructFile.Append(string.Format("                    {0} obj = new {0}();\r\n", fieldItem.FieldType));
                                sbStructFile.Append("                    obj.Serializtion(oByteArray, bSerialize);\r\n");
                                sbStructFile.Append(string.Format("                    {0}[i] = obj;\r\n", fieldItem.FieldName));
                            }
                            else
                            {
                                sbStructFile.Append(string.Format("                    {1}[i] = oByteArray.{0}();\r\n", CodeUtil.GetU3DReadStr(fieldItem, true), fieldItem.FieldName));
                            }
                            sbStructFile.Append("                }\r\n");
                        }
                    }
...
");
                foreach (var fieldItem in stitem.Fields)
                {
                    sbStructFile.Append(string.Format("            {0}\r\n", CodeUtil.GetU3DTypeDefaultValueCode(fieldItem)));
                }
                sbStructFile.Append(@"
        }
");
            ...
            sbProtocolFile.Append("    public partial class PacketVer\r\n");
            sbProtocolFile.Append("    {\r\n");
            sbProtocolFile.Append(string.Format("        public const int Ver = {0};\r\n", oProjectInfo.PacketVer));
            sbProtocolFile.Append(string.Format("        public const int SVer = {0};\r\n", Global.StructOperateVer));
            sbProtocolFile.Append(string.Format("        public const int PVer = {0};\r\n", Global.ProtocolOperateVer));
            sbProtocolFile.Append("    }\r\n\r\n");

            foreach (var ptitem in oProjectInfo.Protocols)
            {
                //protocolStartCode = ptitem.ProtocolCode;
                //请求类生成
                string sProName = "Req" + ptitem.ProtocolName;
                sbProtocolFile.Append(string.Format("    /// <summary>\r\n    ///  [请求类]{0}\r\n    /// </summary>\r\n", ptitem.ProtocolSummary));
                sbProtocolFile.Append(string.Format("    public partial class {0}: Packet \r\n", sProName));
                sbProtocolFile.Append("    {\r\n");
                foreach (var fieldItem in ptitem.ReqFields)
                {
                    ...
                    else if (sTemp == "List")
                    {
                        ...
                        {
                            sbProtocolFile.Append(string.Format("        public {0}[] {1} = new {0}[{2}];\r\n", TypeStr, fieldItem.FieldName, fieldItem.FieldLength));
                        }
                    }
                    else
                    {
                ...
                sbProtocolFile.Append("#if USE_OWNERID\r\n");
                sbProtocolFile.Append("            pkg.OwnerID = OwnerID;\r\n");
                sbProtocolFile.Append("#endif\r\n");
                sbProtocolFile.Append("            pkg.PacketSize = PacketSize;\r\n");
                ...
                foreach (var fieldItem in ptitem.ReqFields)
                {
                    ...
                    else if (sTemp == "List")
                    {
                        ...
                        if (sTemp2 == "Pak")
                        {
                            ...
                            {
                                sbProtocolFile.Append(string.Format("            for (int i = 0; i < {0}; i++)\r\n", fieldItem.FieldLength));
                                sbProtocolFile.Append("            {\r\n");
                                sbProtocolFile.Append(string.Format("                pkg.{1}[i] = {1}[i].Clone() as {0};\r\n", fieldItem.FieldType, fieldItem.FieldName));
                                sbProtocolFile.Append("            }\r\n");
                            }
                        }
                        else
                        {
                            ...
                            else
                            {
                                sbProtocolFile.Append(string.Format("            for (int i = 0; i < {0}; i++)\r\n", fieldItem.FieldLength));
                                sbProtocolFile.Append("            {\r\n");
                                sbProtocolFile.Append(string.Format("                pkg.{0}[i] = {0}[i];\r\n", fieldItem.FieldName));
                                sbProtocolFile.Append("            }\r\n");
                            }
                        }
                    }
                    ...
                    else if (sTemp == "List")
                    {
                        ...
                        if (sTemp2 == "Pak")
                        {
                            if (fieldItem.FieldLength == "*")
                            {
                                ...
                                sbProtocolFile.Append("                {\r\n");
                                sbProtocolFile.Append(string.Format("                      {0}[i].Serializtion(oByteArray, bSerialize);\r\n", fieldItem.FieldName));
                                sbProtocolFile.Append("                }\r\n");
                            }
                            else
                            {
                ...
                foreach (var fieldItem in ptitem.ReqFields)
                {
                    ...
                    else if (sTemp == "List")
                    {
                        ...
                        if (sTemp2 == "Pak")
                        {
                            if (fieldItem.FieldLength == "*")
                            {
                                sbProtocolFile.Append(string.Format("                int {0}Count = (int)oByteArray.ReadUShort();\r\n", fieldItem.FieldName));
                                sbProtocolFile.Append(string.Format("                for (int i = 0; i < {0}Count; i++)\r\n", fieldItem.FieldName));
                                sbProtocolFile.Append("                {\r\n");
                                sbProtocolFile.Append(string.Format("                    {0} obj = new {0}();\r\n", fieldItem.FieldType));
                                sbProtocolFile.Append("                    obj.Serializtion(oByteArray, bSerialize);\r\n");
                                sbProtocolFile.Append(string.Format("                    {0}.Add(obj);\r\n", fieldItem.FieldName));
                                sbProtocolFile.Append("                }\r\n");
                            }
                            else
                            {
                                sbProtocolFile.Append(string.Format("                for (int i = 0; i < Count; i++)\r\n", fieldItem.FieldLength));
                                sbProtocolFile.Append("                {\r\n");
                                sbProtocolFile.Append(string.Format("                    {0} obj = new {0}();\r\n", fieldItem.FieldType));
                                sbProtocolFile.Append("                    obj.Serializtion(oByteArray, bSerialize);\r\n");
                                sbProtocolFile.Append(string.Format("                    {0}[i] = obj;\r\n", fieldItem.FieldName));
                                sbProtocolFile.Append("                }\r\n");
                            }
                        }
                        else
                        {
                            ...
                            else
                            {
                                sbProtocolFile.Append(string.Format("                for (int i = 0; i < {0}; i++)\r\n", fieldItem.FieldLength));
                                sbProtocolFile.Append("                {\r\n");
                                sbProtocolFile.Append(string.Format("                    {1}[i] = oByteArray.{0}();\r\n", CodeUtil.GetU3DReadStr(fieldItem, true), fieldItem.FieldName));
                                sbProtocolFile.Append("                }\r\n");
                            }
                        }
                    }
                ...
                sbProtocolFile.Append(string.Format("    public partial class {0}Creator : IPacketCreator\r\n", sProName));
                sbProtocolFile.Append("    {\r\n");
                    ...
                    else if (sTemp == "List")
                    {
                        ...
                        if (sTemp2 == "Pak")
                        {
                            ...
                            else
                            {
                                sbProtocolFile.Append(string.Format("            for (int i = 0; i < {0}; i++)\r\n", fieldItem.FieldLength));
                                sbProtocolFile.Append("            {\r\n");
                                sbProtocolFile.Append(string.Format("                pkg.{1}[i] = item.Clone() as {0};\r\n", fieldItem.FieldType, fieldItem.FieldName));
                                sbProtocolFile.Append("            }\r\n");
                            }
                        }
                        else
