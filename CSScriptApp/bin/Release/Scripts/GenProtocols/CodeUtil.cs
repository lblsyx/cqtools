...
namespace CSScriptApp.Scripts.GenProtocols
{
    ...
    public static class CodeUtil
    {
        ...
        public static string FieldData2CtorField(FieldInfo fieldData)
        {
        ...
        public static string FieldData2CloneField(FieldInfo fieldData)
        {
            string rlt = string.Empty;//.Format("unknow {0};", fieldData.FieldName);
            ...
            switch (fieldData.FieldType)
            {
                case "string":
                    ...
                    else
                    {
                        ...
                        {
                            rlt = string.Format("for (int i = 0; i < {0}.size(); i++)\r\n", fieldData.FieldName);
                            rlt += "    {\r\n";
        ...
        /// <returns></returns>
        public static string FieldData2CPPField(FieldInfo fieldData)
        {
        ...
        /// <param name="fieldData"></param>
        /// <returns></returns>
        public static string FieldData2CPPSerializeBin(FieldInfo fieldData, string scope)
        {
            string rlt;

            if (string.IsNullOrEmpty(fieldData.FieldLength))
            {//普通变量
                rlt = string.Format("stream << {1}{0};", fieldData.FieldName, scope);
            }
            else
            {
                if (fieldData.FieldLength == "*")
                {
                    rlt = string.Format("ushort us{0}Len = (ushort){0}.size();\r\n        stream << us{0}Len;\r\n        for(int i = 0; i < us{0}Len; ++i)\r\n        {1}\r\n            stream << {3}{0}[i];\r\n        {2}", fieldData.FieldName, "{", "}", scope);
                }
                else
                {
                    rlt = string.Format("for(int i = 0; i < {1}; ++i)\r\n{2}\r\n    stream << {4}{0}[i];\r\n{3}", fieldData.FieldName, fieldData.FieldLength, "{", "}", scope);
                }
            }
        ...
        /// <param name="fieldData"></param>
        /// <returns></returns>
        public static string FieldData2CPPDeserializeBin(FieldInfo fieldData, string scope)
        {
            string rlt;
            ...
            else
            {
                if (fieldData.FieldLength == "*")
                {
                    rlt = string.Format("ushort us{0}Len = 0;\r\n        stream >> us{0}Len;\r\n        for(int i = 0; i < us{0}Len; ++i)\r\n        {1}\r\n            {4} o{0}Item;\r\n            stream >> o{0}Item;\r\n            {0}.push_back(o{0}Item);\r\n        {2}", fieldData.FieldName, "{", "}", scope, fieldData.FieldType);
                }
                else
                {
                    rlt = string.Format("for(int i = 0; i < {1}; ++i)\r\n{2}\r\n    stream >> {4}{0}[i];\r\n{3}", fieldData.FieldName, fieldData.FieldLength, "{", "}", scope);
        ...
        /// <param name="fieldData"></param>
        /// <returns></returns>
        public static string FieldData2CPPSerializeJson(FieldInfo fieldData, string scope)
        {
            string rlt;

            if (string.IsNullOrEmpty(fieldData.FieldLength))
            {
                rlt = string.Format("jsonvalue[\"{0}\"] << {1}{0};", fieldData.FieldName, scope);
            }
            else
            {
                if (fieldData.FieldLength == "*")
        ...
        /// <returns></returns>
        public static string FieldData2CPPDeserializeJson(FieldInfo fieldData, string scope)
        {
            string rlt;
        ...
        public static string FieldData2AS3Field(FieldInfo fieldData)
        {
            ...
            if (string.IsNullOrEmpty(fieldData.FieldLength))
            {
                switch (fieldData.FieldType)
                {
                    ...
                    case "int64":
                    case "uint64":
                        rlt = string.Format("    /**\r\n     * {1}\r\n     */\r\n    public var {0}:Object = 0;", fieldData.FieldName, fieldData.FieldSummary);
                        break;
        ...
        public static string FieldData2AS3Read(FieldInfo fieldData, string scope, string tempVar)
        {
            string rlt = "";
            string pthis = string.IsNullOrEmpty(scope) ? "" : scope + ".";
            ...
            if (string.IsNullOrEmpty(fieldData.FieldLength))
            {//普通变量
                rlt = getAS3ReadCode(fieldData, pthis, false);
            }
            else
            {
                if (fieldData.FieldLength == "*")
                ...
                rlt += "{\r\n";
                rlt += "    " + getAS3ReadCode(fieldData, pthis, true);
                rlt += "\r\n        }";
            }
        ...
        private static string getAS3ReadCode(FieldInfo fieldData, string scope, bool isArray)
        {
            string rlt = "";
            ...
            switch (fieldData.FieldType)
            {
                case "bool":
                    if (isArray)
                    {
                        rlt = string.Format("        {1}{0}.push(bytes.readBoolean());", fieldData.FieldName, scope);
                    }
                    else
                    {
                ...
                case "char":
                    if (isArray)
                    {
                        rlt = string.Format("        {1}{0}.push(bytes.readByte());", fieldData.FieldName, scope);
                    }
                    else
                    {
                ...
                case "uchar":
                    if (isArray)
                    {
                        rlt = string.Format("        {1}{0}.push(bytes.readUnsignedByte());", fieldData.FieldName, scope);
                    }
                    else
                    {
                ...
                case "short":
                    if (isArray)
                    {
                        rlt = string.Format("        {1}{0}.push(bytes.readShort());", fieldData.FieldName, scope);
                    }
                    else
                    {
                ...
                case "ushort":
                    if (isArray)
                    {
                        rlt = string.Format("        {1}{0}.push(bytes.readUnsignedShort());", fieldData.FieldName, scope);
                    }
                    else
                    {
                ...
                case "int":
                    if (isArray)
                    {
                        rlt = string.Format("        {1}{0}.push(bytes.readInt());", fieldData.FieldName, scope);
                    }
                    else
                    {
                ...
                case "uint":
                    if (isArray)
                    {
                        rlt = string.Format("        {1}{0}.push(bytes.readUnsignedInt());", fieldData.FieldName, scope);
                    }
                    else
                    {
                ...
                case "int64":
                    if (isArray)
                    {
                        rlt = string.Format("        {1}{0}.push(bytes.readInt64());", fieldData.FieldName, scope);
                    }
                    else
                    {
                ...
                case "uint64":
                    if (isArray)
                    {
                        rlt = string.Format("        {1}{0}.push(bytes.readUnsignedInt64());", fieldData.FieldName, scope);
                    }
                    else
                    {
                ...
                case "float":
                    if (isArray)
                    {
                        rlt = string.Format("        {1}{0}.push(bytes.readFloat());", fieldData.FieldName, scope);
                    }
                    else
                    {
                ...
                case "double":
                    if (isArray)
                    {
                        rlt = string.Format("        {1}{0}.push(bytes.readDouble());", fieldData.FieldName, scope);
                    }
                    else
                    {
                ...
                case "string":
                    if (isArray)
                    {
                        rlt = string.Format("        {1}{0}.push(bytes.readString());", fieldData.FieldName, scope);
                    }
                    else
                    {
                ...
                case "stream":
                    if (isArray)
                    {
                        ...
                        rlt += string.Format("        {1}{0}.push(stream)", fieldData.FieldName, scope);
                    }
                    else
                    {
                        rlt = "        var len:uint = bytes.readUnsignedShort();\r\n";
                ...
                default:
                    if (isArray)
                    {
                        rlt = string.Format("        {2}{0}.push(PacketUtil.read{1}(bytes));", fieldData.FieldName, fieldData.FieldType, scope);
                    }
                    else
                    {
