...
namespace CSScriptApp.Scripts.GenProtocols
{
    ...
    public class CPPCodeGenerator : IGenerator
    {
        ...
        public void Generate(ProjectInfo oProjectInfo, GeneratorSetting oGeneratorSetting, string sPath1, string sPath2)
        {
            ...
            {
                Program.WriteToConsole("请选择C++协议/结构头文件");
                return;
            }

            if (string.IsNullOrEmpty(sPath2))
            {
                Program.WriteToConsole("请选择C++协议/结构代码文件");
                return;
            }
            ...
            foreach (var stitem in oProjectInfo.Structs)
            {
                ...
                {
                    //声明代码
                    string cppfield = CodeUtil.FieldData2CPPField(fielditem);
                    sbSHFile.Append(string.Format("    {0}\r\n", cppfield));
                    //重置代码
                    string ctorfield = CodeUtil.FieldData2CtorField(fielditem);
                ...
                //结构体序列化/反序列化运算符重载
                sbSSHFile.Append(string.Format("ServerLight::Stream& operator >> (ServerLight::Stream& stream, ST{0}& oST{0});\r\n", stitem.StructName));
                sbSSHFile.Append(string.Format("ServerLight::Stream& operator << (ServerLight::Stream& stream, ST{0}& oST{0});\r\n\r\n", stitem.StructName));
                sbSSHFile.Append(string.Format("Json::Value& operator >> (Json::Value& jsonvalue, ST{0}& oST{0});\r\n", stitem.StructName));
                sbSSHFile.Append(string.Format("Json::Value& operator << (Json::Value& jsonvalue, ST{0}& oST{0});\r\n\r\n", stitem.StructName));

                //结构体序反列化运算符重载
                sbSCFile.Append(string.Format("ServerLight::Stream& operator >> (ServerLight::Stream& stream, ST{0}& oST{0})\r\n", stitem.StructName));
                sbSCFile.Append("{\r\n");
                foreach (var fielditem in stitem.Fields)
                {
                    //反序列化代码
                    string serfield = CodeUtil.FieldData2CPPDeserializeBin(fielditem, string.Format("oST{0}.", stitem.StructName));
                ...
                {
                    //序列化代码
                    string serfield = CodeUtil.FieldData2CPPSerializeBin(fielditem, string.Format("oST{0}.", stitem.StructName));
                    string[] lst = serfield.Split(splitter, StringSplitOptions.RemoveEmptyEntries);
                    serfield = string.Join("    \r\n", lst);
                    sbSCFile.Append(string.Format("    {0}\r\n", serfield));
                }
                ...
                {
                    //反序列化代码
                    string serfield = CodeUtil.FieldData2CPPDeserializeJson(fielditem, string.Format("oST{0}.", stitem.StructName));
                    string[] lst = serfield.Split(splitter, StringSplitOptions.RemoveEmptyEntries);
                    serfield = string.Join("    \r\n", lst);
                    sbSCFile.Append(string.Format("    {0}\r\n", serfield));
                }
                ...
                {
                    //序列化代码
                    string serfield = CodeUtil.FieldData2CPPSerializeJson(fielditem, string.Format("oST{0}.", stitem.StructName));
                    string[] lst = serfield.Split(splitter, StringSplitOptions.RemoveEmptyEntries);
                    serfield = string.Join("    \r\n", lst);
                    sbSCFile.Append(string.Format("    {0}\r\n", serfield));
                }
            ...
            foreach (var ptitem in oProjectInfo.Protocols)
            {
                ...
                foreach (var fielditem in ptitem.ReqFields)
                {
                    ...
                    string serfield = CodeUtil.FieldData2CPPDeserializeBin(fielditem, "");
                    sbBinRead.Append(string.Format("        {0}\r\n", serfield));

                    string ctorfield = CodeUtil.FieldData2CtorField(fielditem);
                    sbCtor.Append(string.Format("    {0}\r\n", ctorfield));

                    string clonefield = CodeUtil.FieldData2CloneField(fielditem);
                    sbClone.Append(string.Format("    {0}\r\n", clonefield));

                    serfield = CodeUtil.FieldData2CPPDeserializeJson(fielditem, "");
                    sbJsonRead.Append(string.Format("        {0}\r\n", serfield));

                    serfield = CodeUtil.FieldData2CPPSerializeBin(fielditem, "");
                    sbBinWrite.Append(string.Format("        {0}\r\n", serfield));

                    serfield = CodeUtil.FieldData2CPPSerializeJson(fielditem, "");
